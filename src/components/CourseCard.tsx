  ...
  return (
    ...
    <div className="text-left px-6 py-6 space-y-3  rounded-xl shadow-md border border-gray-100 hover:shadow-2xl transition transition-delay-15 ease-in-out h-full">
      ...
      >
        ...
        <div>
            <p>Starting from {startingDate.toLocaleString([],{dateStyle:"short"})}</p>
            
            <p>@{startingDate.toLocaleTimeString([], {timeStyle:"short"})+" to "+new Date((startingDate.setHours(startingDate.getHours()+1))).toLocaleTimeString([], {timeStyle:"short"})}</p>
        </div>
      </h5>
